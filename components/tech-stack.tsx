...
export default function TechStack() {
  return (
    <section id="skills" className="py-24 px-4 sm:px-6 lg:px-8 bg-slate-50/80 border-y border-slate-200">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-4xl sm:text-5xl font-bold text-slate-900 mb-4">Technical Skills</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {techCategories.map((category, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group glass p-8 rounded-xl border border-purple-100 hover:border-purple-300 hover:shadow-md hover:shadow-purple-100 transition-all"
            >
              {/* Category Header */}
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className={`font-bold bg-gradient-to-r ${category.color} bg-clip-text text-transparent`}>
                    {category.name}
                  </h3>
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill, i) => (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                    className="px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg text-slate-700 text-sm hover:border-purple-300 hover:text-purple-700 hover:bg-purple-50 transition-all"
                  >
                    {skill}
                  </motion.div>
                ))}
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="mt-16 glass p-8 rounded-2xl border border-purple-100"
        >
          <h3 className="text-2xl font-bold text-slate-900 mb-6 text-center">Real-World Impact</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="text-center">
              <div className="text-3xl font-bold text-purple-600 mb-2">66%</div>
              <div className="text-sm text-slate-600">API Latency Reduction</div>
              <div className="text-xs text-slate-400 mt-1">420ms → 140ms</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-500 mb-2">28%</div>
              <div className="text-sm text-slate-600">Signups Increase</div>
              <div className="text-xs text-slate-400 mt-1">Improved onboarding</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-green-600 mb-2">99.9%</div>
              <div className="text-sm text-slate-600">Uptime Achievement</div>
              <div className="text-xs text-slate-400 mt-1">Autoscaling infrastructure</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-pink-500 mb-2">85%</div>
              <div className="text-sm text-slate-600">Test Coverage</div>
              <div className="text-xs text-slate-400 mt-1">60% fewer defects</div>
            </div>
          </div>
        </motion.div>
        ...
        >
          <h3 className="text-xl font-bold text-slate-900 mb-4">Development Tools</h3>
          <div className="flex flex-wrap justify-center gap-3">
            {['Git & GitHub', 'Postman', 'Figma', 'Jira', 'Notion', 'Asana', 'Canva', 'Tableau', 'VS Code'].map((tool, i) => (
              <span
                key={i}
                className="px-4 py-2 text-sm font-medium text-slate-600 border border-slate-200 bg-white rounded-full hover:bg-purple-50 hover:border-purple-300 hover:text-purple-700 transition-all shadow-sm"
              >
                {tool}
              </span>
            ))}
